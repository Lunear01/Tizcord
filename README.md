...
```
### What is it? 
- Tizcord is a robust, terminal-based concurrent chat application inspired by Discord and built entirely in C. It leverages a multiplexed client-server architecture over TCP sockets to facilitate real-time communication within a command-line interface

### TODO LIST: 
- [x] Login page
- [x] Basic functionality: Establishing connection, sending and receiving messages
- [x] Online status 
- [x] Custom user status
- [x] Channels: Creation, Removal yet to be implemented
    - [x] Only allow admin to create and delete channels
- [x] Private Messages
    - [x] Save DMs to DB
...
- Non-blocking User Interface: Utilizes the ncurses library alongside select() system calls to ensure the terminal interface remains fully responsive during network I/O operations.
- Server and Role Management: Supports server creation, channel administration, and role-based access control for administrative actions such as member moderation.
- Social Systems: Includes a comprehensive friend request system and real-time user status broadcasting.
- Persistent Storage: Implements a SQLite backend for the secure, persistent storage of user credentials, chat histories, and server states, utilizing yescrypt for cryptographic password hashing.
...
Social Commands:
    ...
    - /reject [username] - Decline a friend request.
